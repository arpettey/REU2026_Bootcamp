...
# Summary statistics for observed, imputed-only, and completed Ozone

Yobs <- airquality$Ozone
Ycomp <- data$Ozone

observed_idx <- !is.na(Yobs)
imputed_idx <- is.na(Yobs)

summary_table <- data.frame(
  Group = c("Observed", "Imputed only", "Completed"),
  n = c(
    sum(observed_idx),
    sum(imputed_idx),
    length(Ycomp)
  ...
  Mean = c(
    mean(Yobs[observed_idx]),
    mean(Ycomp[imputed_idx]),
    mean(Ycomp)
  ),
  SD = c(
    sd(Yobs[observed_idx]),
    sd(Ycomp[imputed_idx]),
    sd(Ycomp)
  )
)
...
missing_ozone <- is.na(airquality$Ozone) & !is.na(airquality$Solar.R)

pred <- predict(
  fit,
  newdata = airquality[missing_ozone, ]
)

Yimp <- airquality$Ozone
Yimp[missing_ozone] <- pred
```
...
plot_data <- data.frame(
  ...
  status = ifelse(is.na(airquality$Ozone),
                  "Imputed", "Observed")
)

ggplot(plot_data, aes(Solar.R, Ozone, color = status)) +
  geom_point(size = 2, alpha = 0.85) +
  geom_abline(
...
# Fit regression using observed cases
fit <- lm(Ozone ~ Solar.R, data = airquality)

# Rows where Ozone is missing but Solar.R is observed
missing_rows <- is.na(airquality$Ozone) & !is.na(airquality$Solar.R)

# Predicted values for missing Ozone observations
pred <- predict(
  fit,
  newdata = airquality[missing_rows, ]
)

# Original Ozone values
Yobs <- airquality$Ozone

# Completed dataset after regression imputation
Yimp <- Yobs
Yimp[missing_rows] <- pred

# Summary statistics
Yobs_only <- Yobs[!is.na(Yobs)]
Yimp_only <- pred
Ycomp <- Yimp
...
knitr::include_graphics("ch01-miflow-1.png")
```
...
Oz <- airquality$Ozone
col_vec <- c("red", "blue", "darkgreen", "purple", "orange")

plot(Oz,col="black",type="l",xlab="Day number",ylab="Ozone (ppb)",
     axes = FALSE,lwd = lwd)
lwd <- 0.6
axis(1, lwd = lwd)
axis(2, lwd = lwd, las = 1)
