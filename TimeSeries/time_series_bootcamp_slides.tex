...
\begin{frame}{The seven-step lesson plan}
\begin{enumerate}
  \item Why ordinary regression fails
  \item What makes time series different?
  \item Forecasting mindset
  \item A forecasting competition
  \item Simple forecasting methods
  \item Introducing autoregression
  \item Modern workflow checklist
\end{enumerate}
\end{frame}

\section{1. Why ordinary regression fails}
...
\begin{frame}{Motivating example: temperature and CO$_2$}
...
A naive regression will usually look impressive because both variables have strong time structure. A simple regression that ignores time can confuse trend, persistence, and causal association.


\end{frame}

\begin{frame}{Always Check the Residuals!}

\begin{center}
\includegraphics[scale=.7]{climateResids.pdf}
\end{center}

Both series trend upward over time. Because they share a common trend, a regression of temperature on $\text{CO}_2$ will look impressive on the surface 
